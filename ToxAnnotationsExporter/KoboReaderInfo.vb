...
Public Class KoboReaderInfo
#Region "Private members"
    '      ===============

    Private Shared ReadOnly DATABASE_FILE_RELATIVE_PATH As [String] = ".kobo\KoboReader.sqlite"
                ...
                Using dbConnection As New SQLiteConnection()
                    Try
                        ...
                        Using dbCommand As New SQLiteCommand(dbConnection)
                            Try
                                dbCommand.CommandText = "SELECT ContentID, attribution, Title FROM content WHERE ContentType='6' ORDER BY ContentId" 'file_path equals contentID
                                Using dbResult As SQLiteDataReader = dbCommand.ExecuteReader()
                                    Try
                                        If 3 <= dbResult.FieldCount Then
                                            While True = dbResult.Read()
                                                ...
                                                Dim dbBookFilePath As [String] = dbResult(0).ToString().Substring(20).Replace("/", "\") 'nog verwijderen file:///mnt/onboard/
                                                Dim dbBookThumbnailPath As [String] = ""

                                                Dim dictionaryBookId As [String] = GenerateDictionaryBookId(storageLocation, dbBookId)
                                                If False = readerBooks.ContainsKey(dictionaryBookId) And dbBookId.StartsWith("file:///") Then
                                ...
                                dbCommand.CommandText = "SELECT VolumeID, ChapterProgress,  Text, Annotation,  StartContainerPath, EndContainerPath FROM Bookmark ORDER BY ChapterProgress"
                                Using dbResult As SQLiteDataReader = dbCommand.ExecuteReader()
                                    Try
                                        If 6 <= dbResult.FieldCount Then
                                            ...
                                            While True = dbResult.Read()
                                                ...
                                                Dim dbAnnotationType As [String] = 11 'dbResult(2).ToString()
                                                Dim dbAnnotationMark As [String] = dbResult(2).ToString()
                                                Dim dbAnnotationText As [String] = dbResult(3).ToString()
                                                Dim dbAnnotationFilePath As [String] = "" 'dbResult(5).ToString().Replace("/", "\")
                                                'blobSize = dbResult.GetBytes(4, 0, blobBuffer, 0, blobBufferSize)
                                                Dim dbAnnotationMarkStart As [String] = dbResult(4).ToString 'UTF8.GetString(blobBuffer, 0, CInt(blobSize))
                                                'blobSize = dbResult.GetBytes(5, 0, blobBuffer, 0, blobBufferSize)
                                                Dim dbAnnotationMarkEnd As [String] = dbResult(5).ToString 'UTF8.GetString(blobBuffer, 0, CInt(blobSize))
                                                ...
                                                If True = readerBooks.ContainsKey(dictionaryBookId) Then
                                                    ...
                                                    If AnnotationType.UNKNOWN <> annotationType__1 Then
                                                        ...
                                                        End If
                                                        annotationPage = annotationPage * 100
                                                        ...
                                                        readerBooks(dictionaryBookId).Annotations.Add(New AnnotationInfo(annotationType__1, annotationPage, annotationMarkSourceFileRelativePath, annotationMarkStart, annotationMarkEnd, dbAnnotationMark, _
